...
try:
    ...
    if mode == "prepared":
        ...
        data = data_container("prepared-container")
        ...
        (out / "recreated-seed.json").write_text(json.dumps(dict(old_inode=archived.stat().st_ino,
            new_inode=seed.stat().st_ino, sha256=hashlib.sha256(contents).hexdigest()), indent=2))
        test("testOpenSeed", "open-seed")
    else:
        test("testOpenSeed", "open-seed")
        test("testCreateEditAndReopen", "create-save")
    assert (data_container("final-container") / "Documents/seed.docprobe").read_bytes() == b"0\n"
